...
@NamedQueries({
        @NamedQuery(name = "Menu.findByIgrejaAndFuncionalidades", query = "select m from Menu m left join m.menuPai mp where m.funcionalidade in :funcionalidades and m.igreja.chave = :igreja order by mp.chaveIgreja desc, mp.ordem, m.ordem")
})
