...
@NamedQueries({
    ...
    @NamedQuery(name = "Membro.findFuncionalidadesAcesso", query = "select f from Membro m left join m.acesso a left join a.perfis pr left join pr.funcionalidades fu inner join m.igreja i inner join i.plano p inner join p.funcionalidades f left join i.funcionalidadesAplicativo fa where (f = fu or f = fa) and i.chave = :igreja and m.id = :membro group by f")
})
