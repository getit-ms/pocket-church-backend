...
@NamedQueries({
    ...
    @NamedQuery(name = "Membro.findFuncionalidadesAcesso", query = "select f from Membro m left join m.acesso a left join a.perfis p left join p.funcionalidades f inner join m.igreja.plano pl left join pl.funcionalidades ff where m.id = :membro and m.igreja.chave = :igreja and (ff = f or ff in :aplicativo) group by f")
})
