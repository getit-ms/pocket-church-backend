...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "SentNotification.marcaComoLidoMembro", query = "update tb_sent_notification sn set lido = true from (select chave_igreja, id_membro from tb_sent_notification  where chave_igreja = #igreja and id_membro = #membro order by chave_igreja, id_membro for no key update) upd where sn.chave_igreja = upd.chave_igreja and sn.id_membro = upd.id_membro"),
