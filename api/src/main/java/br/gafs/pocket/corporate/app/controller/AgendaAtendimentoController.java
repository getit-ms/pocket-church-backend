...
import br.gafs.pocket.corporate.app.dto.agendamento.RequisicaoAgendamentoDTO;
...
import br.gafs.pocket.corporate.entity.domain.DiaSemana;
...
public class AgendaAtendimentoController {
    ...
    @JsonView(View.Resumido.class)
    @Produces(MediaType.APPLICATION_JSON)
    public Response buscaGerentes(){
        return Response.status(Status.OK).entity(appService.buscaGerentes()).build();
    }
    ...
    public Response agendar(RequisicaoAgendamentoDTO requisicao){
        return Response.status(Status.OK).entity(appService.agenda(
                requisicao.getColaborador(), requisicao.getHorario(), requisicao.getData())).build();
    }
